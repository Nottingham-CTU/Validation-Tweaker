...
# Test name: t01 Field validation enforcement
import pytest
import time
import json
...
class Test_01_Field_validation_enforcement:
  ...
  def test_01_Field_validation_enforcement(self):
    ...
    self.driver.execute_script("$('#south').remove();dataEntrySubmit('submit-btn-savecontinue')")
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    self.driver.execute_script("//SAVEDESC:Assert successful save")
    assert len(self.driver.find_elements(By.ID, "south")) > 0
    self.driver.find_element(By.NAME, "participant_info_survey_complete").find_element(By.CSS_SELECTOR, "*[value='2']").click()
    self.driver.find_element(By.NAME, "height").send_keys("1.23")
    self.driver.execute_script("//SETDESC:Click into another field")
    ...
    self.driver.find_element(By.NAME, "weight").click()
    self.driver.execute_script("//SETDESC:Assert alert displayed")
    self.driver.find_element(By.XPATH, "//div[contains(@class,'simpleDialog') and contains(text(),'value you provided')]").send_keys("SAVESCREENSHOT")
    assert len(self.driver.find_elements(By.XPATH, "//div[contains(@class,'simpleDialog') and contains(text(),'value you provided')]")) > 0
    ...
    self.driver.execute_script("//SETDESC:Click back into other field")
    self.driver.find_element(By.NAME, "weight").click()
    self.driver.execute_script("//SETDESC:Assert alert displayed")
    self.driver.find_element(By.XPATH, "//div[contains(@class,'simpleDialog') and contains(text(),'value you provided')]").send_keys("SAVESCREENSHOT")
    assert len(self.driver.find_elements(By.XPATH, "//div[contains(@class,'simpleDialog') and contains(text(),'value you provided')]")) > 0
