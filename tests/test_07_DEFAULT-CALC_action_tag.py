...
# Test name: t07 DEFAULT-CALC action tag
import pytest
import time
import json
...
class Test_07_DEFAULT_CALC_action_tag:
  ...
  def test_07_DEFAULT_CALC_action_tag(self):
    ...
    self.driver.find_element(By.NAME, "pmq1").send_keys("15")
    self.driver.execute_script("//SETDESC:Click \"Save & Exit Form\"")
    self.driver.find_element(By.CSS_SELECTOR, "[id=\"submit-btn-saverecord\"]").send_keys("SAVESCREENSHOT")
    self.driver.execute_script("$('#south').remove();dataEntrySubmit('submit-btn-saverecord')")
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    ...
    self.driver.execute_script("//SETDESC:Assert that the field with @DEFAULT-CALC([pmq1]+10) action tag has defaulted to the value 25")
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"tagdefaultcalc\"][value=\"25\"]")) > 0
    self.driver.find_element(By.CSS_SELECTOR, "input[name=\"tagdefaultcalc\"][value=\"25\"]").send_keys("SAVESCREENSHOT")
