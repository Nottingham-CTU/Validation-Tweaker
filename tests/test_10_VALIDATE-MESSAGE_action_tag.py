...
# Test name: t10 VALIDATE-MESSAGE action tag
import pytest
import time
import json
...
class Test_10_VALIDATE_MESSAGE_action_tag:
  ...
  def test_10_VALIDATE_MESSAGE_action_tag(self):
    ...
    self.driver.execute_script("//SETDESC:Assert validation alert displayed and it contains the text \"Test validation message\"")
    ...
    assert len(self.driver.find_elements(By.XPATH, "//div[@aria-describedby='redcapValidationErrorPopup']//div[contains(text(),'Test validation message')]")) > 0
